...
func TestMessageBroker(t *testing.T) {
	...
	msg, err := mb.Consume(ctx, "test", 0)
	if err != nil || string(msg) != "lol" {
		t.Fatal("fail to pop basic value")
	}
	// basic case timeout
	mb.Publish("test", []byte("lol"))
	msg, err = mb.Consume(ctx, "test", time.Second)
	if err != nil || string(msg) != "lol" {
		t.Fatal("fail to pop basic value")
	}
	// fail without timeout
	_, err = mb.Consume(ctx, "test", 0)
	if err == nil {
	...
	for i := 1; i <= 100; i++ {
		...
		go func() {
			defer wg.Done()
			msg, err := mb.Consume(ctx, "test", 10*time.Second)
			if err == nil {
				values[i] = string(msg)
			}
		}()
	...
	_, err = mb.Consume(ctx, "test", time.Millisecond)
	if err == nil {
	...
	msg, err = mb.Consume(ctx, "test", time.Millisecond)
